...
uses
  CPort, Windows, Messages, SysUtils, Classes, Graphics, Controls, Forms,
  Dialogs,
...
begin
  if edit.text <> '' then
    a := messagedlg('Sie haben das Programm ' + edit.lines[0] +
      ' noch nicht gespeichert. Möchten Sie dies jetzt tun ?', mtinformation,
      [mbyes, mbno], 0);
  if a = mryes then
    speichern_DLG;
end;
...
begin
  speichern.showmodal;
  if speichern.modalresult = mrok then
    save(speichern.FileName);
end;
...
begin
  ...
  neu.showmodal;
  edit.text := 'O' + neu.edit1.text + '(' + upstr(neu.edit2.text) + ')' + #13 +
    #10;
...
begin
  ...
  if DTR = 'Enable' then
    StrToDtr := dtrenable
  else if DTR = 'Disable' then
    StrToDtr := dtrdisable
  else if DTR = 'Handshake' then
    StrToDtr := dtrHandshake
  else
  begin
...
begin
  ...
  if rts = 'Enable' then
    StrToRts := rtsEnable
  else if rts = 'Disable' then
    StrToRts := rtsDisable
  else if rts = 'Handshake' then
    StrToRts := rtsHandshake
  else if rts = 'Toggle' then
    StrToRts := rtsToggle
  else
  begin
...
begin
  ...
  if parity = 'Even' then
    StrToParity := prEven
  else if parity = 'Mark' then
    StrToParity := prMark
  else if parity = 'None' then
    StrToParity := prNone
  else if parity = 'Odd' then
    StrToParity := prOdd
  else if parity = 'Space' then
    StrToParity := prSpace
  else
  begin
...
begin
  ...
  if flow = 'Custom' then
    StrToFlow := fcCustom
  else if flow = 'None' then
    StrToFlow := FCnone
  else if flow = 'RtsCts' then
    StrToFlow := fcHardware
  else if flow = 'XonXoff' then
    StrToFlow := fcSoftware
  else
  begin
    messagedlg('fehlerhafte einstellungen für FlowControl', mtError, [mbok],
      0);
    StrToFlow := fcNone;
  end;
end;
...
begin
  ...
  else if stop = '1.5' then
    StrToStop := sbOne5StopBits
  else if stop = '2' then
    StrToStop := sbTwoStopBits
  else
  begin
...
begin
  if com_open then
    messagedlg('Schnittstelle ist noch offen.', mterror, [mbok], 0);
end;
...
begin
  ...
  begin
    typ := radiobutton1.caption;
    typ_index := 1;
  end;
  if radiobutton2.checked = true then
  begin
    typ := radiobutton2.caption;
    typ_index := 2;
  end;
  if radiobutton3.checked = true then
  begin
    typ := radiobutton3.caption;
    typ_index := 3;
  end;
  if radiobutton4.checked = true then
  begin
    typ := radiobutton4.caption;
    typ_index := 4;
  end;
  if radiobutton5.checked = true then
  begin
    typ := radiobutton5.caption;
    typ_index := 5;
  end;
  if radiobutton6.checked = true then
  begin
    typ := radiobutton6.caption;
    typ_index := 6;
  end;
  if radiobutton7.checked = true then
  begin
    typ := radiobutton7.caption;
    typ_index := 7;
  end;
  if radiobutton8.checked = true then
  begin
    typ := radiobutton8.caption;
    typ_index := 8;
  end;
  if radiobutton9.checked = true then
  begin
    typ := radiobutton9.caption;
    typ_index := 9;
  end;
  if radiobutton10.checked = true then
  begin
    typ := radiobutton10.caption;
    typ_index := 10;
  end;
