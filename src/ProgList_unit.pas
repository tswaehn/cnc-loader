...
begin
  if grid.cells[2, arow] = '' then
    dateierweiterung := '*.LOG'
  else
    dateierweiterung := grid.cells[2, arow];
end;
...
begin
  ...
  begin
    form1.inifile.writestring('Verzeichnisse', 'Verzeichnis' + inttostr(a),
      grid.cells[1, a]);
    form1.inifile.writestring('Verzeichnisse', 'Extension' + inttostr(a),
      grid.cells[2, a]);
  end;
...
begin
  ...
  begin
    ...
    if length(grid.cells[1, arow]) > 3 then
      grid.cells[1, arow] := grid.cells[1, arow] + '\';
...
begin
  ...
  begin
    grid.cells[1, a] := form1.inifile.readstring('Verzeichnisse', 'Verzeichnis'
      + inttostr(a), '');
    grid.cells[2, a] := form1.inifile.readstring('Verzeichnisse', 'Extension' +
      inttostr(a), '');
  end;
end;
...
begin
  ...
  begin
    a :=
      messagedlg('Sie haben Änderungen vorgenommen, möchten Sie diese speichern?',
